...
class EditTaskPresenterTest : BasePresenterTest() {
    ...
    @MockK
    private lateinit var validTaskLists: RealmResults<TaskList>
    ...
    fun `Load Task Info - Valid Task - Success`() {
        // arrange
        every { tasksHelper.getTaskAsSingle(validTaskId, any()) } returns Single.just(validTask)
        every { tasksHelper.getTaskLists(any(), any()) } returns Flowable.just(validTaskLists)
        ...
        verify(exactly = 1) { editTaskView.onTaskListsLoaded(taskLists = validTaskLists) }
